  ...
  <!-- add to wishlist  -->
    ...
    $.ajax({
        url: "getWishlistCount.php",  // PHP script to fetch updated count
        method: "GET",  // Use GET method to request the updated count
        success: function(response) {
            // Parse the JSON response
            var data = JSON.parse(response);

            // Update the wishlist count in the header
            $('#wishlistCountValue').text(data.wc); // Update the header
        },
        error: function() {
            alert("Error fetching updated wishlist count.");
        }
    });
    }
    </script>
